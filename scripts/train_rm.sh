#!/bin/bash
# Usage: sbatch --account=$ACCOUNT_NAME ./scripts/train_sft.sh
# source .env && sbatch -A $ACCOUNT_NAME ./scripts/train_sft.sh
...
singularity exec --nv "$IMG" bash -lc '
  ...
  uv sync
  # uv run -m reward_model.train configs/reward_model/llama3_8b_instruct_ep1.yaml
  # uv run -m reward_model.train configs/reward_model/llama3_8b_instruct_ep3.yaml 
  # uv run -m reward_model.train configs/reward_model/llama3_8b_instruct_ep5.yaml
  # uv run -m reward_model.train configs/reward_model/llama3_8b_instruct_ep7.yaml
